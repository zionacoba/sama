-- Add custom_questions array to trips (replaces single custom_question for new trips)
ALTER TABLE trips ADD COLUMN IF NOT EXISTS custom_questions jsonb;

-- Migrate existing single questions into the new array column
UPDATE trips
SET custom_questions = jsonb_build_array(custom_question)
WHERE custom_question IS NOT NULL
  AND trim(custom_question) != '';

-- Add custom_question_answers array to bookings
ALTER TABLE bookings ADD COLUMN IF NOT EXISTS custom_question_answers jsonb;

-- Drop the current 25-parameter signature (param 25 is text)
