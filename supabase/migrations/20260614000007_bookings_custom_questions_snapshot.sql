-- Snapshot the custom question text as-asked onto each booking, so answers stay
-- legible even after the organizer later edits or removes the trip's questions.
--
-- NOTE: This was already applied live (the bookings.custom_questions_snapshot
-- column and the 26-argument book_slot_and_create_booking exist in production).
-- This migration records that change so a `db reset` reproduces the live schema.

ALTER TABLE bookings ADD COLUMN IF NOT EXISTS custom_questions_snapshot jsonb;

-- Drop the prior 25-parameter signature (last param is p_custom_question_answers jsonb)
-- so we can add the trailing p_custom_questions_snapshot parameter.
...
CREATE OR REPLACE FUNCTION book_slot_and_create_booking(
  ...
  p_custom_question_answers  jsonb DEFAULT NULL,
  p_custom_questions_snapshot jsonb DEFAULT NULL
)
RETURNS bigint
LANGUAGE plpgsql
AS $$
...
BEGIN
  ...
  INSERT INTO bookings (
    ...
    p_custom_question_answers,
    p_custom_questions_snapshot
  )
